...
namespace Pegasus_backend.Services
{
    public class LessonGenerateService
    {
        ...
        public IEnumerable<Task> GetTerm(DateTime date, int instance_id=0)
        {
            ...
            foreach (var term in terms)
            {
                ...
                if (day >= 0 && day <= 30)
                {
                    ...
                    yield return Generateone2oneInvoice(next_term,instance_id);
                    yield return GenerateGroupInvoice(next_term, instance_id);
                }

                if (start_date >= term.BeginDate && start_date <= term.EndDate)
                {
                    int this_term = term.TermId;
                    yield return Generateone2oneInvoice(this_term, instance_id);
                    yield return GenerateGroupInvoice(this_term, instance_id);
